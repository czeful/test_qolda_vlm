...
Код для пяти пилотных проверок, описанных в дизайн-документе проекта, плюс
общая инфраструктура (клиент к Qolda, схемы, промпты, метрики), на которой
эти проверки построены. Цель — за несколько дней, без Habitat и без GPU-кластера,
получить количественные данные для решения «идти/не идти» по архитектуре
VLM-навигации на казахском языке, и сразу таблицы, пригодные для статьи
уровня Scopus Q1–Q2.

## Почему именно так устроено

Каждая из пяти проверок — отдельный скрипт с собственным `--help`, который:
пишет сырой лог по каждому вызову модели (`results/*_raw.jsonl` — для
воспроизводимости и ручной проверки), считает агрегированные метрики и
сохраняет их в CSV (`results/*_summary.csv` — то, что идёт в таблицы статьи),
и печатает markdown-таблицу в консоль. `report/make_tables.py` потом собирает
все CSV в один `report/pilot_report.md`.

Всё, включая retry-логику для JSON и метрики калибровки, можно прогнать
**без GPU и без поднятого сервера** флагом `--mock` — он подставляет
`MockQoldaClient`, который возвращает правдоподобные, но синтетические
ответы. Это не заменяет реальный эксперимент, но проверяет, что весь
конвейер (парсинг JSON, retry, расчёт метрик, запись CSV) действительно
работает, прежде чем тратить время GPU. Все пять скриптов уже прогнаны
с `--mock` в этом репозитории — `python main.py all --mock` проходит от
начала до конца без ошибок.
